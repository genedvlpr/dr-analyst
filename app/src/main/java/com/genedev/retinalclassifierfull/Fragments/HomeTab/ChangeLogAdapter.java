...
public class ChangeLogAdapter extends ArrayAdapter<ChangeLog> {
    private static final String LOG_TAG = ChangeLogAdapter.class.getSimpleName();
     ...
     * @param changeLog A List of AndroidFlavor objects to display in a list
     */
    public ChangeLogAdapter(Activity context, List<ChangeLog> changeLog  ) {
        // Here, we initialize the ArrayAdapter's internal storage for the context and the list.
        // the second argument is used when the ArrayAdapter is populating a single TextView.
        // Because this is a custom adapter for two TextViews and an ImageView, the adapter is not
        // going to use this second argument, so it can be any value. Here, we used 0.
        super(context, 0, changeLog  );
    }
    /**
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        ChangeLog changeLog = getItem(position);
        ...
        if (convertView == null) {
            convertView = LayoutInflater.from(getContext()).inflate(R.layout.home_changelog_adapter, parent, false);
        }

        TextView changeLogsText = (TextView) convertView.findViewById(R.id.subtitle);
        changeLogsText.setText(changeLog.vchangeLog);
